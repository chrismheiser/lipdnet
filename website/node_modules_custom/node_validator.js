    ...
    validate: (function(files, options, cb){
        ...
        var verifyBibJson = function(v){
          try {
            ...
            console.log("before fixauthor");
            v = fixAuthor(v);
            console.log("after fixauthor");
            for (var _p = 0; _p < v.length; _p++){
              var _pub = v[_p];
              _crumbs = "publication" + _p;
              // loop for each item in this pub
              for(var _key in _pub){
                if(_pub.hasOwnProperty(_key)){
                  console.log(_key);
                 if (_arrs.includes(_key)){
                   var _isArrObj = verifyArrObjs(_crumbs + "." + _key, _pub[_key], true);
                 } else if (_key === "journal"){
                   verifyDataType("object", _crumbs + "." + _key, _pub[_key], false);
                 } else {
                   verifyDataType("string", _crumbs + "." + _key, _pub[_key], false);
                 }
                }
              }
            }
        ...
        var requiredColumnsCtMatch = function (filename, columns) {
          ...
          try{
            for (var _h = 0; _h < columns.length; _h++){
              // skip null and empty entries.
              if(columns[_h]){
                if(Array.isArray(columns[_h].number)){
                  console.log("column is an array");
                  // console.log(columns[_h]);
                  metaCt += columns[_h].number.length;
                } else {
                  console.log("column is not an array");
                  console.log(columns[_h]);
                  metaCt += 1;
                }
