...
gladstone.createBagDirectory =  function (args) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      fs.mkdir(procArgs.bagName, function (err) {
          if (err) {
              console.error(strings.errorBagCreation);
              console.log("gladstone: createBagDirectory: " + err);
          }
          ...
          // console.log("bagname: " + procArgs.bagName);
          setTimeout(function () {
...
gladstone.createTagmanifest = function(args){
  var procArgs = processArgs(args);
  return new Promise(function (resolve, reject) {
    try{
      // console.log("processing tagmanifest");
      gladstone.createManifest(procArgs.bagName, procArgs, "tagmanifest");
      setTimeout(function () {
        // console.log("resolving tagmanifest");
          resolve(true);
      }, 200);
    } catch(err){
      console.log("gladstone: createTagmanifest: " + err);
    }
  });
};
...
gladstone.copyOriginToData = function (args) {
    ...
    //     if (err) throw err;
    // });
    console.log("Copy files from origin to /data...");
...
gladstone.getRelativePath = function (filePath) {
  ...
  if (filePath.indexOf("data") === -1){
    relName = path.basename(filePath);
  } else {
    var relPath = filePath.replace(/\\\\/g, '').replace(/\//g, '/');
    splitName = relPath.split('/data/');
    relName = relPath.replace(splitName[0], '');
    ...
    if (type === 'manifest') {
        recursive(myPath, function (err, files) {
            files.forEach(function (file) {
              // console.log("Creating manifest hash: " + file);
      ...
      files.forEach(function(file){
         if(path.extname(file) === ".txt" && file) {
           gladstone.createFileHash(path.join(myPath, file), args, manifestFileName);
         }
      });
    }
};
...
gladstone.createFileHash = function (file, args, manifestFileName) {
    ...
    console.log("Creating file hash: " + file);
    try{
      ...
      console.log("gladstone: createFileHash: " + err);
    }
};
