...
## BAGIT WRAPPER
##
## Creates a layer on top of the python bagit
## module that focuses on tasks we want.
###############################################
...
###############################################
## BAGIT MODULE
## All original bagit module code is below
###############################################
...
class Bag(object):
    ...
    def _validate_entries(self, processes):
        ...
        if not available_hashers:
            raise RuntimeError(
                "%s: Unable to validate bag contents: none of the hash algorithms in %s are supported!" % (
                self, self.algs))
...
class ChecksumMismatch(ManifestErrorDetail):
    ...
    def __str__(self):
        ...
        self.path, self.algorithm, self.expected, self.found)
