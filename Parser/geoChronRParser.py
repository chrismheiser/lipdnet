...
########################## CHRONOLOGY WORKSHEET ###############################
        ...
        input_data = [dataSheet.cell_value(11, 1), dataSheet.cell_value(11,1)]

        with open('folders/' + new_file_name_csv, 'w') as test_file:

            csv_writer = csv.writer(test_file, delimiter = ',')
            for i in range(len(input_data)):
                csv_writer.writerows([x[i] for x in input_data[i]])
