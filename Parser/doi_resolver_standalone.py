...
CHANGELOG
Version 1.3 Mon, 11.23.15 by Chris
...
Output: '-doi.lpd' file that has populated 'pub' fields resolved from the DOI (if there is one) parsed from the
original .lpd file.
...
class DOIResolver(object):
    ...
    """
    Sometimes DOI is string, sometimes list, sometimes empty. This checks for all cases and returns list of 1+ DOI id's
    """
    ...
    def run_standalone(self):
        ...
        directory = '/Users/chrisheiser1/Desktop/lpd-library/'
        os.chdir(directory)
        ...
        for txt in file_list:
            ...
            # Check if the file is a Bagit Archive
            print("is it a zip file??")
            print(self.check_zip(txt))
            ...
            json_data = open(name + '/data/' + name + '.jsonld')
            ...
            print(data['pub'][0]['identifier'][0]['id'])
            file_out = open(directory + name + '/data/' + name + '-mod.jsonld', 'w+')
            # empty, doi_list = self.multi_doi(data['pub']['identifier']['id'])
            empty, doi_list = self.multi_doi(data['pub'][0]['identifier'][0]['id'])
            ...
            if not empty:
                for doi in doi_list:
                    ...
                    output = self.get_data(data['pub'][0], self.clean(doi))
